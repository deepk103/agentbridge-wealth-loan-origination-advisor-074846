...
resource "azurerm_resource_group" "main" {
  name     = "rg-agentbridge-${var.environment}-wealth-loan-origination-advisor"
  location = var.region
  tags = {
    Environment = var.environment
    ManagedBy   = "terraform"
    Blueprint   = "Wealth Loan Origination Advisor"
  }
}

# Virtual Network
resource "azurerm_virtual_network" "main" {
  name                = "vnet-agentbridge-wealth-loan-origination-advisor"
...
resource "azurerm_subnet" "main" {
  name                 = "snet-agentbridge-wealth-loan-origination-advisor"
  resource_group_name  = azurerm_resource_group.main.name
...
resource "azurerm_network_security_group" "main" {
  name                = "nsg-agentbridge-wealth-loan-origination-advisor"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_public_ip" "main" {
  ...
  name                = "pip-agentbridge-wealth-loan-origination-advisor"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_network_interface" "main" {
  name                = "nic-agentbridge-wealth-loan-origination-advisor"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_linux_virtual_machine" "main" {
  name                  = "vm-agentbridge-wealth-loan-origination-advisor"
  resource_group_name   = azurerm_resource_group.main.name
  location              = azurerm_resource_group.main.location
...
resource "azurerm_managed_disk" "data" {
  ...
  name                 = "disk-agentbridge-wealth-loan-origination-advisor-data"
  location             = azurerm_resource_group.main.location
  resource_group_name  = azurerm_resource_group.main.name
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_cognitive_account" "ai_services" {
  ...
  name                  = "agentbridge-wealth-loan-origination-advisor-ai"
  location              = azurerm_resource_group.main.location
  resource_group_name   = azurerm_resource_group.main.name
  kind                  = "AIServices"
  sku_name              = "S0"
  custom_subdomain_name = "agentbridge-wealth-loan-origination-advisor-ai"
...
resource "azurerm_user_assigned_identity" "vm_foundry" {
  count               = var.enable_foundry ? 1 : 0
  name                = "agentbridge-wealth-loan-origination-advisor-vm-identity"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
}
...
resource "null_resource" "foundry_agents" {
  ...
  provisioner "local-exec" {
    interpreter = ["bash", "-c"]
    command     = <<-EOT
      ENDPOINT="https://agentbridge-wealth-loan-origination-advisor-ai.services.ai.azure.com"
